...
// //↖↑↗裏返し実行用（プレイヤー1用）
// //numにはオセロの位置、kには裏返し先のオセロの方向を示す数値（絶対値）を入れる
// function BJMinus(num,k) {
//     if(num -k*2 > 0 && pieceSta[num -k*2] !== 0) { //2つ先のコマが存在するか確認
//         for(i = num - k*2; i>=1 && i<=36; i -= k) { //2つ先以上のマスに、黒いコマがあるか順に確認していく
//             if(pieceSta[i] === 1) { //2つ先以上のマスについて、黒いコマがあれば稼働（※1）
//                 blackAdd(num - k); //1つ先のコマを黒にする（※2）
//                 placeable = 1; //裏返すコマがあるので1に変更
//                 for(j = i + k; j < num; j += k) { //※1と※2の間に、白いコマがあればそちらも裏返す
//                     if(pieceSta[j] === 2) {
//                         blackAdd(j);
//                     }
//                     console.log(`BJMinus-jkadou：${j}`); //確認用2
...
// //↙↓↘裏返し実行用（プレイヤー1用）
// //numにはオセロの位置、kには裏返し先のオセロの方向を示す数値（絶対値）を入れる
// function BJPlus(num,k) {
//     if(num +k*2 <= 36 && pieceSta[num +k*2] !== 0) { //2つ先のコマが存在するか確認
//         for(i = num + k*2; i>=1 && i<=36; i += k) { //2つ先以上のマスに、黒いコマがあるか順に確認していく
//             if(pieceSta[i] === 1) { //2つ先以上のマスについて、黒いコマがあれば稼働（※1）
//                 blackAdd(num + k); //1つ先のコマを黒にする（※2）
//                 placeable = 1; //裏返すコマがあるので1に変更
...
// //↖↑↗裏返し実行用（プレイヤー2用）
// //numにはオセロの位置、kには裏返し先のオセロの方向を示す数値（絶対値）を入れる
// function WJMinus(num,k) {
//     if(num -k*2 > 0 && pieceSta[num -k*2] !== 0) { //2つ先のコマが存在するか確認
//         for(i = num - k*2; i>=1 && i<=36; i -= k) { //2つ先以上のマスに、白いコマがあるか順に確認していく
//             if(pieceSta[i] === 2) { //2つ先以上のマスについて、白いコマがあれば稼働（※1）
//                 console.log(num - k); //確認用1
//                 whiteAdd(num - k); //1つ先のコマを白にする（※2）
//                 placeable = 1; //裏返すコマがあるので1に変更
//                 for(j = i + k; j < num; j += k) { //※1と※2の間に、黒いコマがあればそちらも裏返す
//                     if(pieceSta[j] === 1) {
//                         whiteAdd(j);
//                     }
//                     console.log(`WJMinus-jkadou：${j}`); //確認用2
...
// //↖↑↗←裏返し実行用（プレイヤー1用）
// function blackUraMinus(k,num) {
//     i = num + k;
//     for(i = num + k*2; i>=0 && i<=36; i += k) {
//         if(pieceSta[i] === 1) {
//             blackAdd(i - k);
//             for(j = i - k*2; j>=0 && j < num; j -= k) {
//                 if(pieceSta[j] === 2) {
//                     blackAdd(j);
//                 }
//                 console.log(`b-jkadou-minus：${j}`);
//             }
//         }
//         console.log(`b-ikadou-minus：${i}`);
//     }
// }

// //→↙↓↘裏返し実行用（プレイヤー1用）
// function blackUraPlus(k,num) {
//     i = num + k;
//     for(i = num + k*2; i>=0 && i<=36; i += k) {
//         if(pieceSta[i] === 1) {
//             blackAdd(i - k);
//             for(j = i - k*2; j > num && j <= 36; j -= k) {
//                 if(pieceSta[j] === 2) {
//                     blackAdd(j);
//                 }
//                 console.log(`b-jkadou-plus：${j}`);
//             }
//         }
//         console.log(`b-ikadou-plus：${i}`);
//     }
// }

// //↖↑↗←裏返し実行用（プレイヤー2用）
// function whiteUraMinus(k,num) {
//     i = num + k;
//     for(i = num + k*2; i>=0 && i<=36; i += k) {
//         if(pieceSta[i] === 2) {
//             whiteAdd(i - k);
...
//                     whiteAdd(j);
//                 }
//                 console.log(`w-jkadou-minus：${j}`);
//             }
//         }
//         console.log(`w-ikadou-minus：${i}`);
//     }
// }

// //→↙↓↘裏返し実行用（プレイヤー2用）
// function whiteUraPlus(k,num) {
//     i = num + k;
//     for(i = num + k*2; i>=0 && i<=36; i += k) {
//         if(pieceSta[i] === 2) {
//             whiteAdd(i - k);
...
//                     whiteAdd(j);
//                 }
//                 console.log(`w-jkadou-plus：${j}`);
//             }
//         }
//         console.log(`w-ikadou-plus：${i}`);
//     }
// }
...
//         if(pieceSta[index+1] === 0) {
//             if(user === 1) {
//                 console.log('click1'); //確認用
//                 canvas = document.querySelector(`.canvas${index+1}`);
//                 ctx = canvas.getContext('2d');
//                 ctx.beginPath();
//                 ctx.arc(31,31,28,0,360,0); //黒円を作成
//                 ctx.fill();
//                 pieceSta[index+1] = 1; //コマの状態変更
//                 user = 2; //user変更
//                 document.querySelector('.teban').textContent = 'プレイヤー2（白）の番です';
//             } else if (user === 2) {
//                 console.log('click2'); //確認用
//                 canvas = document.querySelector(`.canvas${index+1}`);
//                 ctx = canvas.getContext('2d');
//                 // ctx.clearRect(0, 0, canvas.width, canvas.height);
//                 ctx.beginPath();
//                 ctx.arc(31,31,28,0,360,0); //白円を作成
//                 ctx.stroke();
//                 pieceSta[index+1] = 2; //コマの状態変更
//                 user = 1; //user変更
//                 document.querySelector('.teban').textContent = 'プレイヤー1（黒）の番です';
...
// x、y計算用の関数を作成する→いらなくなった
// function xyCalc(subject) {
//     // subX = subject.getBoundingClientRect().x + 35;
//     // subY = subject.getBoundingClientRect().y + 35;
//     subX = subject.getBoundingClientRect().x -282;
//     subY = subject.getBoundingClientRect().y -113;
//     return {x:subX,y:subY}; //オブジェクトとして出力
// }
